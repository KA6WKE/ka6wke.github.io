...
successfully. The instance is running, the security group allows HTTP on port 80, the Internet
Gateway is attached, and the route table has a working route to the internet. But the page still
won't load.
...
3. Select **Upload a template file** and upload <a href="lab-02-nacl.yaml" download>lab-02-nacl.yaml</a>
4. Enter a stack name (e.g., `brokenlabs-vpc-lab-02`) and click **Next** > **Next** > **Submit**
...
The instance is running and healthy. The security group allows HTTP. The route table has a
valid route to the Internet Gateway. The page still never arrives.

## Fix the Lab

The security group and route table both look correct. Something else in the VPC networking
stack is filtering traffic before it reaches the instance.
...
- [Add and remove rules](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html#Rules)
